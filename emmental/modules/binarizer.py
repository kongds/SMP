...
class ThresholdBinarizer(autograd.Function):
    ...
    @staticmethod
    def forward(ctx, inputs: torch.tensor, threshold: float, sigmoid: bool, limit: bool = True):
        """
        Args:
...
class ZeroBinarizer(autograd.Function):
    """
    Thresholdd binarizer.
    Computes a binary mask M from a real value matrix S such that `M_{i,j} = 1` if and only if `S_{i,j} > \tau`
    where `\tau` is a real value threshold.
    ...
    @staticmethod
    def forward(ctx, inputs: torch.tensor, limit: bool = True):
        """
        Args:
            inputs (`torch.FloatTensor`)
                The input matrix from which the binarizer computes the binary mask.
        ...
        Returns:
            mask (`torch.FloatTensor`)
                ...
                retained, 0 - the associated weight is pruned).
        """
        mask = (inputs > 0).type(inputs.type())
        return mask
